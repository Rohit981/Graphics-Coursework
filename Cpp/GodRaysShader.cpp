...
GodRaysShader::GodRaysShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"GodRays_vs.cso", L"GodRays_PS.cso");
}
...
void GodRaysShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	D3D11_BUFFER_DESC matrixBufferDesc;
	D3D11_SAMPLER_DESC samplerDesc;
	D3D11_SAMPLER_DESC samplerRayDesc;
	D3D11_BUFFER_DESC godraysBuffer;
	...
	godraysBuffer.Usage = D3D11_USAGE_DYNAMIC;
	godraysBuffer.ByteWidth = sizeof(GodRayBufferType);
	godraysBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	godraysBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	godraysBuffer.MiscFlags = 0;
	godraysBuffer.StructureByteStride = 0;
	renderer->CreateBuffer(&godraysBuffer, NULL, &godRayBuffer);
}

void GodRaysShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture1, Light* light, XMFLOAT2 screenSize, const XMMATRIX& viewMatrix2, const XMMATRIX& ProjecMatrix2)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;

	MatrixBufferType* dataPtr;
	XMMATRIX tworld, tview, tview2, tproj, tproj2;
	...
	tview2 = XMMatrixTranspose(viewMatrix2);
	tproj2 = XMMatrixTranspose(ProjecMatrix2);
	...
	godPtr->worldMatrix = tworld;
	godPtr->viewMatrix = tview2;
	godPtr->projectionMatrix = tproj2;
